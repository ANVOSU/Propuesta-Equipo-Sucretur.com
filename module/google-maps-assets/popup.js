(function($) {
    ...
    let $deactivateForm  = jQuery('#google-maps-deactivateForm');
    ...
    let deactivateFormSubmitted = localStorage.getItem('google-maps-deactivateFormSubmitted');
    ...
    function submitForm(deactivate_options, id) {
        ...
        data.push({name: 'action', value: 'google-maps-deactivate'});
        ...
        if (!deactivateFormSubmitted) {
            localStorage.setItem('google-maps-deactivateFormSubmitted', true);
            deactivateFormSubmitted = true;

            jQuery.post('./controller/extension/module/elfsight_google_maps.php', jQuery.param(data)).then(function(result) {
